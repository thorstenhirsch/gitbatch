...
func (m *Model) renderOverview() string {
	...
	// Render title
	title := m.styles.Title.Render(fmt.Sprintf(" Matched Repositories (%d) ", len(m.repositories)))
	
	// Render repositories
	var lines []string
	for i := startIdx; i < endIdx; i++ {
		r := m.repositories[i]
		line := m.renderRepositoryLine(r, i == m.cursor)
...
func (m *Model) renderRepositoryLine(r *git.Repository, selected bool) string {
	// Status indicator
	statusIcon := " "
	...
	// Build the line
	parts := []string{statusIcon, name}
	if branchName != "" {
		parts = append(parts, branchInfo)
	}
	if syncInfo != "" {
		parts = append(parts, syncInfo)
	}
...
func (m *Model) renderStatusBar() string {
	...
	spacing := totalWidth - leftWidth - rightWidth - centerWidth
	if spacing < 0 {
		spacing = 0
	}
	...
	return m.styles.StatusBar.Width(totalWidth).Render(statusText)
}
...
Views:       b  branches    c  commits    r  remotes
             s  status      S  stash      Tab  back to overview
